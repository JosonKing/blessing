...
actionTip = "\n+-------------------------------------------------------------------+\n| add: 添加 | rm: 移除 | addall: 添加所有 | rmall:移除所有 |\n|+----------------------------------------------------------------+\nlsm: 查看所有模块 | lss: 查看已选模块 | lsa: 查看所有模块及已选模块 |\n+---------------------------------------------------------+\n|  done: 完成   |  exit: 取消   |   help: 查看指令   |\n+---------------------------------------------------------+"
...
def handleAction(sourcePath, action):
  while True:
    if action == 'addall':
      ...
      packResponse = pack()
      if packResponse == 'cancleDialog':
        action = input(actionResponse)
      elif packResponse == 'success':
        break
      else:
        print("打包失败，请重新操作\n")
        print(actionTip)
